...
> **AI translation.** This English edition was machine-generated from the Traditional
> Chinese original. If the two disagree, the original is authoritative.

These rules refer to the [World Mahjong Championship rules][1] (the site is now gone).

# Composition of Mahjong Tiles #
1. This ruleset uses a set of 136 mahjong tiles, without flower tiles.
2. The 136 tiles consist of 34 kinds, 4 of each.
3. Tiles of the same kind have the same face and the same function; this is called a "kind". Tiles of different kinds must be clearly distinguishable from one another.
4. The 34 kinds split into "number tiles" and "honor tiles", or into "orphan" tiles and "simple" tiles.
...
1. In a round, each of the four players holds a wind position, going counter-clockwise in the order East, South, West, North.
2. The dealer is the East player; the other three are collectively the "non-dealers".
3. Seat honor: the honor tile matching one's own wind position is the "seat honor", also called own wind; the remaining wind tiles are "guest honors".
4. Passing the deal: after each round, the former South player becomes the dealer of the new round.
5. Cycle: every four rounds form a cycle.
6. Keeping the deal: if the winner of the round is the dealer, the deal does not pass; this is "keeping the deal". The next round is called the "first keep"; keeping the deal grants no bonus points.
7. A game may leave keeping the deal disabled; competitive rules must not enable it.

# Game Flow #

## Setup ##
Every game begins with a seating draw; every round begins with breaking the wall and shuffling.

### Position Roll ###
Rolling the dice means rolling two six-sided dice numbered 1 to 6.

Starting the count at 1 on the roller and going counter-clockwise, count up to the total rolled; the final position is the "position roll". That is: 5 and 9 land on the roller; 2, 6, and 10 on the next seat; 3, 7, and 11 on the opposite seat; 4, 8, and 12 on the previous seat.

### Seating Draw ###
The procedure is as follows:
1. The four players first sit down in any order.
2. Take the four honor tiles East, South, West, North, shuffle them face down, and lay them in a single row with their long edges flush against each other.
...
By length, games split into the full match (four cycles: East, South, West, North), the half match (two cycles: East, South), and the East-cycle match (the East cycle only).
The cycle honor serves only as a record-keeping aid; changing it requires no new seating draw and does not affect passing the deal, and every round still begins with the East player drawing and discarding.

## Round ##
A round begins with the shuffle and ends with a win or a washout.

### Shuffle ###
For a hand shuffle, all mahjong tiles are placed face down and the four players thoroughly mix them by hand.
Each player takes 34 tiles, stacking two tiles into one stack, and arranges 17 stacks long-edge to long-edge into a row called the wall.
Once built, push the rows toward the center so the four walls form an approximate square, completing the shuffle.

### Breaking the Wall ###
1. The dealer rolls the dice and counts the position roll to determine which player is the "wall opener".
2. The wall opener rolls the dice and counts the position roll to determine in front of which player's wall the drawing begins.
3. Using the dealer's roll count, the wall opener pushes off that many stacks from the far right end of the wall toward the left, setting them aside as the "wall tail". The portion to the left of the divide is the "wall head".
...
2. In turn, each player draws one tile from the wall head and then discards one tile into the river. The river is arranged neatly six tiles per row; once a row of six is full, continue on a second row. Before the round ends, no tile in the river may be taken except by a "declaration".
3. After someone makes a "declaration", once the declarer discards a tile, the draw-and-discard resumes from their next seat.
...
## Basic Melds ##
1. Sequence: three tiles of the same suit with consecutive numbers form a "sequence".
2. Triplet: three tiles of the same kind form a "triplet".
3. Quartet: four tiles of the same kind become a "quartet" after a "kong"; without a kong they do not count as a quartet.
4. Pair: two tiles of the same kind are a "pair".

## Open and Concealed Melds ##
If every tile of a meld was drawn, it is a concealed meld: the "concealed sequence", "concealed triplet", and "concealed quartet". A concealed quartet still requires a kong but cannot be robbed. If one of a meld's tiles was claimed when another player put it into the river, it is an open meld: the "open sequence", "open triplet", and "open quartet".

## Scoring ##
To score, take the laid-out winning shape and sum the scores of all patterns the winning shape satisfies; within the same category, only the highest-scoring qualifying pattern counts. See the score table for pattern scores.

# Detailed Rules and Penalties #

## Deal ##
1. Misroll: if there is an error during the dice roll, re-roll immediately once noticed. If the four players have already begun taking tiles, the roll already made stands.
2. Illegal wall break: if the wall is broken at the wrong place, correct it immediately. But if all four players have already taken four tiles, this break stands.
3. Illegal dealer: if someone wrongly acts as dealer and starts taking tiles from the wall, and it is caught before the correct East player takes tiles, reshuffle and re-deal. If caught before the correct East player's third draw, continue dealing until all four players have 12 tiles, then have the correct East player take the skip tile and proceed by the normal procedure. If it is noticed only after the correct East player has finished the third draw, that player loses the right to deal this round; the wrongful dealer serves as dealer for this round and continues it. After this round ends, the deal passes normally to the seat after the correct East player.
4. Wrong tiles taken: if someone takes the wrong tiles during the deal, correcting it before they look at their hand or touch the tile faces avoids penalty; otherwise those stacked tiles become live penalty tiles.
5. Exposing a wall tile: if someone accidentally flips a wall tile during the deal, and those are tiles they were to take, treat it as an exposed hand; otherwise reshuffle and re-deal.
6. Deal dead hand: if before the first discard into the river the deal count is found short, tiles may be taken from the wall head to make it up; if before looking at the hand or touching tile faces the deal count is found excessive, a referee or the opposite seat randomly removes the excess tiles and places them at the wall head to correct it. All other cases are treated as a long or short hand.
...
1. For simultaneous declarations, priority from highest to lowest is:
   1. a win by the discarder's next seat
   2. a win by the discarder's opposite seat
   3. a win by the discarder's previous seat
   4. pung or kong
   5. chow
   When multiple players win simultaneously, take the single highest-priority player by the above order, called a "priority win". Aside from wins, if a chow declarer has already revealed the open sequence after declaring chow, other players lose the right to pung or kong that tile.
2. Only a legal declaration has priority; against an incorrect declaration, a lower-priority legal declaration can cancel that incorrect declaration.
3. A declaration is judged by the announcement; if one does not announce, players may ignore the action and continue draw-and-discard.
4. An open meld formed by a declaration may not be moved again until the round ends, except to declare an added kong.
